...
import { Clock, MapPin, Activity, Flame, Trophy, CalendarDays, Brain, Heart, TrendingUp, TrendingDown, Medal, X, ChevronRight, ChevronLeft, Zap } from 'lucide-react'; 
...
const StatsPage = () => {
  ...
  useEffect(() => {
    const loadActivities = async () => {
      try {
        ...
        if (stored) {
          setActivities(stored); // JSON.parse больше не нужен!
        }
      } catch (error) {
  ...
  const totalCalories = filteredActivities.reduce((sum, a) => {
    const duration = parseInt(a.duration || 0);
    let multiplier = 8;
    const intensity = (a.intensity || '').toLowerCase();
    
    if (intensity.includes('niska')) multiplier = 5;
    else if (intensity.includes('wysoka')) multiplier = 12;
  ...
  return (
    <>
        ...
        <div className="bg-[#13072E]/40 backdrop-blur-xl border border-purple-500/20 rounded-3xl p-6 shadow-[0_8px_30px_rgba(147,51,234,0.1)]">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center p-3 rounded-2xl bg-[#0B0316]/50 border border-purple-500/10 hover:border-purple-500/30 transition-colors">
              ...
              <div className="flex-1">
                <p className="text-[10px] font-bold uppercase text-purple-400/60 tracking-wider mb-0.5">Spalone kalorie</p>
                <p className="text-white font-black text-lg leading-none">
                  {totalCalories.toLocaleString()} <span className="text-purple-300/50 text-xs font-bold ml-0.5">kcal</span>
                </p>
              </div>
            </div>
      ...
      {/* МОДАЛЬНОЕ ОКНО КАРУСЕЛЬ (ДЕТАЛИ ТРЕНИРОВКИ) ВЫНЕСЕНО СЮДА */}
      {selectedDayActivities && selectedDayActivities.length > 0 && (
        <div className="fixed inset-0 z-[140] flex items-center justify-center px-4 bg-[#0B0316]/90 backdrop-blur-md">
          <div className="bg-[#13072E] border border-purple-500/40 rounded-[2rem] p-6 w-full max-w-sm shadow-[0_0_60px_rgba(168,85,247,0.5)] animate-modal-pop text-center relative overflow-hidden">
            ...
            {/* Навигация (Стрелки) если больше 1 тренировки */}
            ...
            {/* Контент активной тренировки */}
            <div className="relative z-10 px-6">
              
              <div className="mb-6">
                {selectedDayActivities.length > 1 && (
                  <div className="text-[9px] font-bold text-fuchsia-400/80 mb-2 uppercase tracking-widest">
                    Trening {currentActivityIndex + 1} z {selectedDayActivities.length}
                  </div>
                )}
                <h3 className="text-2xl font-black text-white">
                  {selectedDayActivities[currentActivityIndex].customName || selectedDayActivities[currentActivityIndex].type}
                </h3>
              ...
              <div className="grid grid-cols-2 gap-3 text-left mb-6">
                <div className="bg-[#0B0316]/50 border border-purple-500/10 rounded-xl p-3">
                  <p className="text-[10px] font-bold uppercase text-purple-400/60 mb-1">Czas</p>
                  <p className="text-white font-bold">{selectedDayActivities[currentActivityIndex].duration} min</p>
                </div>
                <div className="bg-[#0B0316]/50 border border-purple-500/10 rounded-xl p-3">
                  <p className="text-[10px] font-bold uppercase text-purple-400/60 mb-1">Intensywność</p>
                  <p className="text-white font-bold">{selectedDayActivities[currentActivityIndex].intensity}</p>
                </div>
                {selectedDayActivities[currentActivityIndex].distance && (
                  <div className="bg-[#0B0316]/50 border border-purple-500/10 rounded-xl p-3">
                    <p className="text-[10px] font-bold uppercase text-purple-400/60 mb-1">Dystans</p>
                ...
                {selectedDayActivities[currentActivityIndex].mood && (
                  <div className="bg-[#0B0316]/50 border border-purple-500/10 rounded-xl p-3">
                    <p className="text-[10px] font-bold uppercase text-purple-400/60 mb-1">Nastrój</p>
